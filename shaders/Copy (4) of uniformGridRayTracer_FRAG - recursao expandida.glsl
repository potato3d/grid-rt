...
// Shading data
const vec3 g_background = vec3( 0.0, 0.0, 1.0 );
...
{
	g_hitPos = g_rayOrig + v_rayDir * g_hit.distance;
...
vec3 shadePhong()
{
	...
	vec3 specularVector = normalize( reflect( v_rayDir, g_hitNormal ) );
	...
	v_rayDir  = directionTowardsLight;
...
vec3 shadeHeadlight()
{
	...
	vec3 rayDirNormalized = normalize( v_rayDir );
...
bool rayTrace()
{
    ...
    vec3 dirSignBits;
    dirSignBits.x = ( v_rayDir.x < 0.0 )? 1.0 : 0.0;
    dirSignBits.y = ( v_rayDir.y < 0.0 )? 1.0 : 0.0;
    dirSignBits.z = ( v_rayDir.z < 0.0 )? 1.0 : 0.0;
	...
	vec3 startPoint = g_rayOrig + v_rayDir * tnear;
...
{
	g_rayOrig = g_hitPos + g_hitNormal * 1e-3;
	v_rayDir = reflect( v_rayDir, g_hitNormal );
}
...
bool setupRefractionRay( vec3 rayDir, float ratio )
{
	...
	return v_rayDir.x != 0.0 || v_rayDir.y != 0.0 || v_rayDir.z != 0.0;
}
...
void main()
{
	...
	// Entering
	bool haveRefraction = setupRefractionRay( previousRayDir, 1.0 / g_refractIndex );
	if( haveRefraction )
	{
		foundHit = rayTrace();
		...
		else
		{
			...
			bool haveRefraction = setupRefractionRay( previousRayDir, g_refractIndex / 1.0 );
			if( haveRefraction )
			{
				foundHit = rayTrace();
...
//		g_rayOrig = g_hitPos + g_hitNormal * 1e-3;
//		v_rayDir = reflect( primaryRayDir, g_hitNormal );
//
//		g_hit.distance = MAX_VALUE;
